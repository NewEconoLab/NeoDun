...
{
		HAL_FLASH_Unlock();					//解锁
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,faddr,value);
		HAL_FLASH_Lock(); 
}
...
{
		HAL_FLASH_Unlock();					//解锁
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE,faddr,value);
		HAL_FLASH_Lock();           //上锁
}
...
void STMFLASH_Read_ByteArray(uint32_t ReadAddr,uint8_t *pBuffer,uint32_t NumToRead)
{
	uint32_t i;
	for(i=0;i<NumToRead;i++)
	{
