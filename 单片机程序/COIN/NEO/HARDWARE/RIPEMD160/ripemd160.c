...
long long _count = 0;
uint32_t _stateMD160[5] = {0x67452301,0xefcdab89,0x98badcfe,0x10325476,0xc3d2e1f0};
uint32_t _blockDWords[16];
uint32_t _buffer[64];
...
{
		memset(_blockDWords,0,16);
		memset(_buffer,0,64);
		memset(_stateMD160,0,sizeof(_stateMD160));
		_count = 0;
...
void _HashData(uint8_t *partIn, int ibStart, int cbSize)
{
		...
		{	
				...
				{			
						_buffer[i+bufferLen] = partIn[i+partInBase];
				}	
